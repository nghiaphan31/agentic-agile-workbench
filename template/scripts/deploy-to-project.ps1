...
# --- Chemins ---
$WorkbenchRoot = Split-Path -Parent $PSScriptRoot  # template/
$WorkbenchVersion = Get-Content (Join-Path (Split-Path -Parent $WorkbenchRoot) "VERSION") -Raw
$WorkbenchVersion = $WorkbenchVersion.Trim()

# Fichiers et dossiers à copier depuis template/
$TemplatePath = $WorkbenchRoot
$FilesToCopy = @(
    ".roomodes",
    ".clinerules",
    ".workbench-version",
